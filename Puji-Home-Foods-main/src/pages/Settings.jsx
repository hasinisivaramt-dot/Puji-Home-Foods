    ...
    <div style={{ minHeight: '100vh', background: C.cream, padding: '80px 0 0' }}>
      <div style={{ maxWidth: '1250px', margin: '0 auto', background: '#fff', borderRadius: 0, boxShadow: '0 8px 30px rgba(0,0,0,.08)', overflow: 'hidden' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '220px 1fr', minHeight: 'calc(100vh - 80px)' }}>
          ...
          {/* ── Sidebar ── */}
          <div style={{ background: 'linear-gradient(180deg,#220000,#4d0000)', color: '#fff', padding: '20px', display: 'flex', flexDirection: 'column' }}>
...
function Section({ icon, title, emoji, children }) {
  return (
    <div style={{ background: '#fff', border: '1px solid #eee', borderRadius: 16, padding: '28px 32px', marginBottom: 24, boxShadow: '0 2px 10px rgba(0,0,0,.04)' }}>
      <h2 style={{ color: '#3D0000', display: 'flex', alignItems: 'center', gap: 10, marginBottom: 22, fontSize: '1.1rem' }}>
        {emoji ? <span>{emoji}</span> : icon} {title}
      </h2>
      {children}
    </div>
  )
