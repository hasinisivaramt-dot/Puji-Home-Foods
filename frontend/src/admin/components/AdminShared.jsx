...
export function Sidebar({
  ...
  window.innerWidth <= 768
    ? (mobileMenu ? AC.sidebarW : 64)
    : (collapsed ? 64 : AC.sidebarW),
  flexShrink: 0,
        ...
        {menuItems.map(item => {
          ...
          return (
            <button key={item.key} onClick={() => onNav(item.key)}
              ...
              style={{ width:'100%', display:'flex', alignItems:'center', gap:10, padding:
  window.innerWidth <= 768
    ? '12px 16px'
    : (collapsed ? '12px 0' : '10px 14px'),
  ...
  <span>{item.label}</span>
      ...
      <div style={{ padding:'12px', borderTop:'1px solid rgba(201,168,76,.15)', flexShrink:0 }}>
        ...
        >
          ...
          {(window.innerWidth <= 768 || !collapsed) && 'Logout'}
        </button>
      </div>
    </div>
  )
...
const notifRef = useRef(null)
  ...
  style={{
    display: "flex",
    alignItems: "center",
    gap: window.innerWidth <= 768 ? ".5rem" : "1rem",
    flex: 1,
    minWidth: 0,
  }}
>
...
<h1
  style={{
    ...
    color: "#1a0400",
    margin: 0,
    fontSize: window.innerWidth <= 768 ? "1.1rem" : "2.2rem",
    whiteSpace: "nowrap",
    overflow: "hidden",
...
>
  ...
  style={{
    display: 'flex',
    alignItems: 'center',
    gap: window.innerWidth <= 768 ? '.4rem' : '1rem',
    minWidth: 0
  }}
>
          {/* Date Range */}
          ...
          <div ref={profileRef} style={{ position:'relative' }}>
            <button onClick={() => { setProfileOpen(o => !o); setNotifOpen(false) }}
              style={{ display:'flex', alignItems:'center', gap:8, background:'none', border:`1px solid ${profileOpen ? AC.gold : 'rgba(201,168,76,.2)'}`, borderRadius:24, padding: window.innerWidth <= 768
  ? '4px'
  : '4px 12px 4px 4px', cursor:'pointer', transition:'all .2s' }}
