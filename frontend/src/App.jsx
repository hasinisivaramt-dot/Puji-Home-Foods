...
// ── Data ─────────────────────────────────────────────────────────
// backgroundPosition tuned per image so the food is always centred & fully visible
const HERO_SLIDES = [
...
const NAV_LINKS = ['Home','Categories','Best Sellers','About Us','Gallery','Reviews','Contact']
...
// ── Gold hover style helper ───────────────────────────────────────
const goldHoverStyle = {
  boxShadow: '0 0 0 1.5px #C9A84C, 0 16px 40px rgba(201,168,76,0.18), 0 4px 16px rgba(0,0,0,0.10)',
  borderColor: '#C9A84C',
}
...
// ── Section Header ────────────────────────────────────────────────
function SectionHeader({ label, title, gold, sub, dark }) {
  const tc = dark ? C.white : '#1a0400'
  return (
    <div style={{ textAlign: 'center', marginBottom: '3.5rem' }}>
      ...
      <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: 'clamp(2.1rem,3.8vw,3rem)', fontWeight: 800, color: tc, marginBottom: 10 }}>
        {title} <span style={{ color: C.gold }}>{gold}</span>
      </h2>
      {sub && <p style={{ color: dark ? 'rgba(240,230,208,.6)' : '#7a4020', fontSize: '1rem', maxWidth: 520, margin: '0 auto', fontWeight: 400 }}>{sub}</p>}
    </div>
  )
}
    ...
    <nav style={{
        ...
        {NAV_LINKS.map(l => (
          <li key={l}>
            <a
              ...
              style={{ display: 'block', padding: '.45rem .72rem', textDecoration: 'none', fontSize: '.8rem', fontWeight: 500, color: C.cream, transition: 'color .3s', position: 'relative' }}
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '.5rem', flexShrink: 0 }}>
        ...
        >
          ...
          {totalItems > 0 && (
            <span style={{ position: 'absolute', top: -4, right: -4, background: C.crimson, color: 'white', borderRadius: '50%', width: 17, height: 17, fontSize: 9, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', border: `1.5px solid ${C.brown}`, transition: 'transform .2s', transform: 'scale(1)' }}>
...
function Hero() {
  ...
  return (
    <section id="home" style={{ position: 'relative', height: '100vh', minHeight: 580, display: 'flex', alignItems: 'center', overflow: 'hidden' }}>
      {/* Slides — cropped to fit full width */}
      {HERO_SLIDES.map((s, i) => (
        <div key={i} style={{
...
function FeatureBar() {
  const items = [
    ['No Preservatives', <svg key="np" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#C9A84C" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>],
    ['100% Homemade',    <svg key="hm" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#C9A84C" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/></svg>],
    ['Premium Quality',  <svg key="pq" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#C9A84C" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>],
    ['Fast Delivery',    <svg key="fd" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#C9A84C" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="1" y="3" width="15" height="13"/><polygon points="16 8 20 8 23 11 23 16 16 16 16 8"/><circle cx="5.5" cy="18.5" r="2.5"/><circle cx="18.5" cy="18.5" r="2.5"/></svg>],
  ]
  return (
    <div style={{ background: 'rgba(26,4,0,.92)', backdropFilter: 'blur(20px)', borderTop: '1px solid rgba(201,168,76,.22)', borderBottom: '1px solid rgba(201,168,76,.22)', padding: '16px 0' }}>
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 2rem', display: 'flex', alignItems: 'center', justifyContent: 'space-around', flexWrap: 'wrap', gap: 12 }}>
        {items.map(([txt, icon], i) => (
          <>
            {i > 0 && <div key={`sep${i}`} style={{ width: 1, height: 28, background: 'rgba(201,168,76,.22)' }} />}
            <div key={txt} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '5px 12px', borderRadius: 8, cursor: 'default', transition: 'all .3s' }}
              onMouseEnter={e => e.currentTarget.style.background = 'rgba(201,168,76,.1)'}
              onMouseLeave={e => e.currentTarget.style.background = 'transparent'}
            >
              {icon}
              <span style={{ fontSize: '.83rem', fontWeight: 500, color: C.cream }}>{txt}</span>
            </div>
          </>
        ))}
      </div>
...
function AboutUs() {
  ...
  return (
    <section id="about-us" style={{ padding: '90px 0', background: '#fff' }}>
          ...
          <Reveal style={{ position: 'relative' }}>
            <div
              ...
              style={{
                ...
                boxShadow: imgHov
                  ? '0 0 0 3px rgba(201,168,76,.2), 0 30px 70px rgba(0,0,0,.18)'
            ...
            >
              ...
              <div style={{
                position: 'absolute', inset: 0,
                background: 'linear-gradient(to top, rgba(42,16,5,.45) 0%, transparent 55%)',
                pointerEvents: 'none',
              }} />
              {/* Floating badge */}
              <div style={{
                position: 'absolute', bottom: 20, left: 20,
                background: 'rgba(26,4,0,.82)',
          ...
          <Reveal delay={0.15}>
            <div>
              ...
              <h2 style={{
                ...
                color: '#1a0400', marginBottom: '1.4rem',
              }}>
              ...
              {[
                ...
                <p key={i} style={{
                  fontSize: '.93rem', lineHeight: 1.8,
                  color: '#5a2e10', marginBottom: '1rem',
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: hov
          ? '0 0 0 1.5px #C9A84C, 0 8px 24px rgba(201,168,76,.14)'
    ...
    >
      ...
      <span style={{
        ...
        color: hov ? C.crimson : '#3a1a05',
        letterSpacing: '.2px', transition: 'color .3s',
        fontFamily: "'DM Sans', sans-serif",
      }}>{stat.label}</span>
    </div>
  )
...
function Categories({ setSelectedCategory }) {
  ...
  style={{
    padding: '88px 0',
    background: 'linear-gradient(135deg, #2B0000, #5C0000, #7A0000)',
    color: 'white'
  }}
>
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 2rem' }}>
...
// ── Products ──────────────────────────────────────────────────────
function Products({ filteredProducts, searchTerm, setSearchTerm }) {
...
function WhyCard({ w }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ marginBottom: '1.1rem', animation: 'subtleFloat 3.5s ease-in-out infinite', filter: 'drop-shadow(0 0 6px rgba(201,168,76,.2))' }}>
        {w.icon}
      </div>
...
>
      ...
      {lb && (
        <div onClick={() => setLb(null)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.92)', backdropFilter: 'blur(12px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999 }}>
          ...
          <img src={lb} style={{ maxWidth: '85vw', maxHeight: '80vh', borderRadius: 16, objectFit: 'contain' }} alt="preview" />
        </div>
      )}
    </section>
  )
...
function Footer() {
  return (
    <footer id="contact" style={{ background: `linear-gradient(180deg,${C.brown} 0%,#080100 100%)`, borderTop: '1px solid rgba(201,168,76,.2)', position: 'relative', overflow: 'hidden' }}>
...
export default function App() {
  // page: 'home' | 'cart' | 'checkout' | 'success' | 'failure'
  ...
  if (page === 'checkout') {
    return (
      <>
        ...
        <Checkout
          onBack={() => setPage('cart')}
          onSuccess={(form) => { setOrderInfo(form); setPage('success') }}
          onFailure={() => setPage('failure')}
        />
      </>
    )
  }
