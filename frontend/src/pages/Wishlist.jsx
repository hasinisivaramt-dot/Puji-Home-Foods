...
>
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#FAF7F2', paddingTop: 80 }}>
      ...
      {/* Header */}
      <div className="wishlist-header" style={{ background: `linear-gradient(135deg,${C.maroon},#5C0000)`, padding: '3rem 2rem 2.5rem', textAlign: 'center', position: 'relative' }}>
        <button
...
>
  ...
  style={{
    ...
    padding: window.innerWidth <= 480 ? '.85rem' : '1rem',
    display: 'flex',
    flexDirection: 'row',
    gap: window.innerWidth <= 480 ? '.85rem' : '1rem',
    alignItems: 'flex-start'
  }}
>
                  <div
  style={{
    width: window.innerWidth <= 480 ? 84 : window.innerWidth <= 768 ? 92 : 100,
    height: window.innerWidth <= 480 ? 84 : window.innerWidth <= 768 ? 92 : 100,
    borderRadius: 14,
    overflow: 'hidden',
    flexShrink: 0,
...
>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 6, gap: 8 }}>
                      <div style={{ minWidth: 0 }}>
                        <div style={{ fontFamily: "'Playfair Display', serif", fontWeight: 700, fontSize: window.innerWidth <= 480 ? '.92rem' : '1.02rem', color: '#1a0400', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: window.innerWidth <= 480 ? 'nowrap' : 'normal' }}>{p.name}</div>
                        <div style={{ fontSize: '.72rem', color: '#9a6040', marginTop: 2 }}>{p.category} · Base ₹{p.price}</div>
                      </div>
                      <button onClick={() => removeFromWishlist(p.id)}
                        style={{ background: '#faf5ee', border: 'none', color: '#b08a6a', fontSize: '.95rem', cursor: 'pointer', padding: '4px 8px', borderRadius: 8, transition: 'all .2s', lineHeight: 1, flexShrink: 0 }}
                        onMouseEnter={e => { e.currentTarget.style.color = '#e74c3c'; e.currentTarget.style.background = '#fdeceb' }}
                        onMouseLeave={e => { e.currentTarget.style.color = '#b08a6a'; e.currentTarget.style.background = '#faf5ee' }}
                        title="Remove">✕</button>
                    </div>
                    ...
                    <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 10 }}>
                      {WEIGHT_OPTIONS.map(wt => (
                        <button key={wt} onClick={() => setWeight(p.id, wt)}
                          style={{ padding: window.innerWidth <= 480 ? '3px 9px' : '4px 11px', borderRadius: 20, border: `1.5px solid ${w === wt ? C.gold : 'rgba(201,168,76,.25)'}`, background: w === wt ? `linear-gradient(135deg,${C.crimson},${C.darkRed})` : 'transparent', color: w === wt ? 'white' : '#7a4020', fontSize: '.7rem', fontWeight: 600, cursor: 'pointer', transition: 'all .2s' }}>
...
>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: window.innerWidth <= 480 ? 'space-between' : 'flex-start', gap: 8 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                          <button onClick={() => setQty(p.id, q - 1)}
                            style={{ width: 26, height: 26, borderRadius: '50%', border: `1.5px solid ${q <= 1 ? 'rgba(201,168,76,.2)' : C.gold}`, background: 'transparent', color: q <= 1 ? 'rgba(201,168,76,.3)' : C.gold, fontSize: '.95rem', fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: q <= 1 ? 'not-allowed' : 'pointer' }}>−</button>
                          <span style={{ minWidth: 20, textAlign: 'center', fontWeight: 700, fontSize: '.88rem', color: '#1a0400' }}>{q}</span>
                          <button onClick={() => setQty(p.id, q + 1)}
                            style={{ width: 26, height: 26, borderRadius: '50%', border: `1.5px solid ${C.gold}`, background: `linear-gradient(135deg,${C.crimson},${C.darkRed})`, color: 'white', fontSize: '.95rem', fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>+</button>
                        </div>
                        <span style={{ fontFamily: "'Playfair Display', serif", fontWeight: 700, fontSize: '1.05rem', color: C.crimson, marginLeft: 4 }}>₹{pr * q}</span>
                      </div>
                      <button onClick={() => handleAddOne(p)}
                        style={{ padding: '9px 20px', borderRadius: 50, border: 'none', background: addedIds[p.id] ? '#2a7a2a' : `linear-gradient(135deg,${C.crimson},${C.darkRed})`, color: 'white', fontWeight: 600, fontSize: '.82rem', cursor: 'pointer', transition: 'all .3s', boxShadow: '0 4px 12px rgba(107,15,15,.25)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, width: window.innerWidth <= 480 ? '100%' : 'auto' }}>
          ...
          {/* RIGHT — order summary */}
          <div className="wishlist-summary" style={{ position: 'sticky', top: 96 }}>
